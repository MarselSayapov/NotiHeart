...
public sealed class NotificationWorker : BackgroundService
{
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation("Sms worker started on queue {Queue}", _rabbitOptions.QueueName);
    ...
    private async Task OnMessageReceivedAsync(object sender, BasicDeliverEventArgs args)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning(
                    "SMS attachments ignored {NotificationId} Count {Count}",
            ...
            await _smsSender.SendAsync(message.Recipient, message.Text, CancellationToken.None);
        ...
        catch (TemporarySmsException ex) when (message is not null)
        {
        ...
        catch (PermanentSmsException ex) when (message is not null)
        {
