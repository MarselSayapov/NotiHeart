...
public sealed class EmailChannelWorker : BackgroundService, IDisposable
{
        ...
        ILogger<EmailChannelWorker> logger)
    {
        ...
        _routingKey = NotificationRouting.GetRoutingKey(NotificationChannel.Email);
        _queueName = NotificationRouting.GetQueueName(NotificationChannel.Email);
        _retryQueueName = NotificationRouting.GetRetryQueueName(NotificationChannel.Email);
        _deadLetterQueueName = NotificationRouting.GetDeadLetterQueueName(NotificationChannel.Email);
        _deadLetterExchange = NotificationRouting.GetDeadLetterExchange(NotificationChannel.Email);
    ...
    private async Task HandleMessageAsync(BasicDeliverEventArgs args, CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            attempt.Error = "Simulated email delivery failure.";
            attempt.FinishedAt = DateTimeOffset.UtcNow;
