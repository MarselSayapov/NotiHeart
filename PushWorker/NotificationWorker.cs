...
public sealed class NotificationWorker : BackgroundService
{
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation("Push worker started on queue {Queue}", _rabbitOptions.QueueName);
    ...
    private async Task OnMessageReceivedAsync(object sender, BasicDeliverEventArgs args)
    {
        ...
        try
        {
            ...
            var attachmentNames = await LoadAttachmentNamesAsync(message.AttachmentIds, CancellationToken.None);
            var payload = new PushPayload
            {
                DeviceToken = message.Recipient,
                Text = message.Text,
                AttachmentNames = attachmentNames
            };

            await _pushSender.SendAsync(payload, CancellationToken.None);
        ...
        catch (TemporaryPushException ex) when (message is not null)
        {
        ...
        catch (PermanentPushException ex) when (message is not null)
        {
    ...
    private async Task<IReadOnlyCollection<string>> LoadAttachmentNamesAsync(Guid[] attachmentIds, CancellationToken cancellationToken)
    {
        ...
        var names = await dbContext.NotificationAttachments
            .AsNoTracking()
